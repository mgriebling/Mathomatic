 ...
 */
void organize(token_type	*equation,	/* equation side pointer */
...
static inline void org_up_level(token_type	*bp, token_type *ep, int level, int invert)
{
	if (invert) {
		for (; bp <= ep; bp++) {
			bp->level--;
 ...
 */
static int org_recurse(token_type	*equation,	/* equation side pointer */
					   int			*np,		/* pointer to length of equation side */
					   int loc, int level, int *elocp)
{
	token_type	*p1, *bp, *ep;
	int		op, sub_op;
	int		i;
 ...
 */
void elim_loop(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
			   int			*np)		/* pointer to length of equation side */
{
	if (abort_flag) {
 ...
 */
void simp_ssub(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
			   int			*np,		/* pointer to length of equation side */
 ...
 */
void simp_equation(int	n)	/* equation space number to simplify */
{
 ...
 */
void mid_simp_side(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
				   int			*np)		/* pointer to length of equation side */
{
 ...
 * Trivial factoring is done.
 */
void mid_simp_equation(int	n)	/* equation space number to simplify */
{
	if (empty_equation_space(n))
		return;
 ...
 */
void simps_side(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
				int			*np,		/* pointer to length of equation side */
 ...
 */
void simpv_side(token_type	*equation,	/* pointer to the beginning of equation side to factor */
				int			*np,		/* pointer to length of equation side */
 ...
 * Trivial factoring is done.
 */
void simpv_equation(int	 n,	/* equation space number to simplify */
 ...
 */
int factor_imaginary(token_type	*equation,	/* pointer to the beginning of equation side */
 ...
 * not in the divisor.
 */
void factorv(token_type	*equation,	/* pointer to the beginning of equation side to factor */
			 int		*np,		/* pointer to equation side length */
 ...
 */
void calc_simp(token_type	*equation,
			   int		*np)
{
 ...
 */
void approximate(token_type	*equation,
				 int		*np)
{
	if (repeat_flag) {
		/* do more */
 ...
 */
int simp_i(token_type	*equation,
				 int	*np)
{
	int	i;
	int	level;
 ...
 */
void
simp_divide(token_type	*equation,
			int			*np)
{
	do {
		do {
 ...
 */
void simp2_divide(token_type	*equation,
				  int			*np,
				  long			v,
				  int			fc_level)
{
 ...
 */
static int simpb_vcmp(const void *v1, const void *v2)
{
 ...
 */
void simpb_side(token_type	*equation,	/* pointer to the beginning of equation side */
				int			*np,		/* pointer to length of equation side */
				...
				int			fc_level)	/* factor constants code, passed to factor_constants() */
{
	...
	if (vc) {
		...
		qsort(va, vc, sizeof(*va), simpb_vcmp);
 ...
 */
void simple_frac_side(token_type	*equation,	/* pointer to the beginning of equation side */
					  int			*np)		/* pointer to length of equation side */
{
	if (*np == 1) {
		make_simple_fractions(equation, np);
 ...
 * Globals tlhs[] and trhs[] are wiped out.
 */
void simpa_side(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
 ...
 */
void simpa_repeat_side(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
					   int			*np,		/* pointer to length of the equation side */
 ...
 */
void simpa_repeat(int	n,			/* equation space number to simplify */
				  int	quick_flag,	/* "simplify quick" option, simpler fractions with no (x+1)^2 expansion */
				  int	frac_flag)	/* "simplify fraction" option, simplify to the ratio of two polynomials */
{
	if (empty_equation_space(n))
 ...
 * Globals tes[], tlhs[], and trhs[] are wiped out.
 */
void simple_frac_repeat_side(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
							 int		*np)		/* pointer to length of the equation side */
{
	if (*np <= 0)
		return;
 ...
 */
int simp_loop(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
			  int			*np)		/* pointer to length of equation side */
{
	int	i;
	int	rv = false;
 ...
 */
int simp_pp(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
			int			*np)
{
	int		i, j, k;
	int		ilevel, jlevel;
 ...
 * Return true if equation side was modified.
 */
int integer_root_simp(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
					  int			*np)
{
	int	modified = false;
	int	i, j;
 ...
 * Return true if equation side was modified.
 */
int simp_constant_power(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
						int			*np)
{
	int	i, j;
	int	level;
 ...
 * Return true if equation side was modified.
 */
int simp2_power(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
				int			*np)
{
	int	i, i1, j, k;
	int	level;
 ...
 * Return true if equation side was modified.
 */
int combine_constants(token_type	*equation,	/* pointer to the beginning of equation side */
					  int			*np,		/* pointer to length of equation side */
 ...
 */
int calc(int	*op1p,	/* Pointer to operator 1, which comes immediately before operand 1 if this operator exists. */
...
static int const_recurse(token_type	*equation,
						 int *np, int loc, int level, int iflag)
{
	int		loc1, old_loc;
	int		const_count = 0;
	int		op;
 ...
 */
int elim_k(token_type	*equation,	/* equation side pointer */
		   int			*np)		/* pointer to length of equation side */
{
 ...
 */
int se_compare(token_type	*p1,			/* first sub-expression pointer */
			   long			n1,				/* first sub-expression length */
			   token_type	*p2,			/* second sub-expression pointer */
			   long			n2,				/* second sub-expression length */
			   int			*diff_signp)	/* different sign flag pointer */
{
	...
	l1 = min_level(p1, n1);
	l2 = min_level(p2, n2);
	rv = compare_recurse(p1, (int)n1, l1, p2, (int)n2, l2, diff_signp);
#if	DEBUG
 ...
 */
static int compare_recurse(token_type	*p1,
						   int n1, int l1,
						   token_type *p2,
						   int n2, int l2, int *diff_signp)
{
	...
	for (pv1 = p1;;) {
		...
		for (i = 0;; i++) {
			...
			if (compare_recurse(pv1, len, (pv1->level <= l1) ? l1 : (l1 + 1),
			    opa2[i], (int)(opa2[i+1] - opa2[i]) - 1, (opa2[i]->level <= l2) ? l2 : (l2 + 1), &j)) {
				switch (op1) {
				case 0:
				case TIMES:
				case DIVIDE:
 ...
 * Return true if equation side was modified.
 */
int elim_sign(token_type	*equation,
			  int			*np)
{
	int		i, j, k;
	int		level;
 ...
 * Return true if equation side was modified.
 */
int div_imaginary(token_type	*equation,
				  int			*np)
{
	int		i, j, k;
	int		n;
 ...
 */
int reorder(token_type	*equation,
			int			*np)
{
	return(order_recurse(equation, np, 0, 1));
}

static void swap(token_type	*equation,
				 int		*np,
				 int		level,
				 int		i1, int i2)
{
 ...
 */
static int order_recurse(token_type	*equation,
						 int		*np, int loc, int level)
{
	int	i, j, k, n;
	int	op = 0;
	int	modified = false;
 ...
 */
int rationalize(token_type	*equation,
				int		*np)
{
	int	i, j, k;
	int	i1, k1;
