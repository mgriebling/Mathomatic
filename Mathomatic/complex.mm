...
do_repeat:
	if (*cp == '\0') {
		my_strlcpy(prompt_str, "Enter root (positive integer): ", sizeof(prompt_str));
		if ((cp = get_string(buf, sizeof(buf))) == NULL)
			return false;
	}
	root = strtod(cp, (char **)&cp);
 ...
 */
int complex_root_simp(token_type *equation,	/* equation side pointer */
 ...
 */
int approximate_complex_roots(token_type *equation,	/* equation side pointer */
							  int		 *np)		/* pointer to length of equation side */
{
	int	rv = false;
 ...
 */
int get_constant(token_type	*p1,	/* expression pointer */
 ...
 */
int parse_complex(token_type *p1,	/* expression pointer */
				  int		   n,	/* length of expression */
