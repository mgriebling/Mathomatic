...
#include "includes.h"
//
//@interface unfactor : NSObject
//
//@end


//@implementation unfactor
 ...
 */
static void no_divide(token_type *equation, int *np) {
	int	i, j;
	int	level;
...
static int usp_sub(token_type *equation, int *np, int i) {
	int	level;
	int	j;
 ...
 * Return true if equation side is modified.
 */
int unsimp2_power(token_type *equation, int *np) {
	int	modified = false;
	int	i;
...
int usp2_sub(token_type	*equation, int *np, int i) {
	int	level;
	int	j, k;
