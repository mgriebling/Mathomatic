...
static int flist_recurse(token_type *p1, int n, int out_flag, char *string, size_t sbuffer_size, int line, int pos, int cur_level, int *highp, int *lowp);
 ...
 */
void default_color(int	set_no_color_flag)	/* If true, set no color even if text_color is set.  Normally this would be false. */
{
	if (html_flag != 2 && gfp != stdout) {
		return;
	}
 ...
 */
static void trim_zeros(char	*buf)
{
	long j;
 ...
 */
int list1_sub(int	n,				/* equation space number */
 ...
 * or zero on failure.
 */
int list_sub(int	n)	/* equation space number */
{
	if (empty_equation_space(n))
		return 0;
 ...
 */
char * var_name(long	v)	/* Mathomatic variable */
{
	char	*cp = NULL;
	long	l;
 ...
 */
int list_var(long	v,			/* variable to convert */
 ...
 */
int list_proc(token_type	*p1,			/* expression pointer */
 ...
 */
char * list_equation(int	n,				/* equation space number */
					 int	export_flag)	/* flag for exportable format (usually false) */
{
	int	len;
	char	*cp;
 ...
 */
char * list_expression(token_type	*p1,		/* expression pointer */
					   int		n,				/* length of expression */
					   int		export_flag)
{
	int	len;
 ...
 */
int list_string(token_type	*p1,		/* expression pointer */
				int			n,			/* length of expression */
...
int list_string_sub(token_type	*p1,			/* expression pointer */
					int			n,				/* length of expression */
 ...
 */
int int_expr(token_type	*p1,		/* expression pointer */
			 int		n)		/* length of expression */
{
	int	i;
	int	rv = 1;
 ...
 */
int list_code_equation(int					en,			/* equation space number */
 ...
 * Return string, or NULL if error.
 */
char * string_code_equation(int					en,			/* equation space number */
							enum language_list	language,
							int					int_flag)	/* integer arithmetic flag */
{
	int	len;
 ...
 * Return length of output (number of characters).
 */
int list_code(token_type			*equation,	/* equation side pointer */
 ...
 */
char * flist_equation_string(int	n)	/* equation space number */
{
	int	i;
	long len, cur_len, buf_len;
	int	pos;
	int	high = 0, low = 0;
		...
		if (n_rhs[n]) {
			if (cur_line == 0) {
				...
				if ((current_columns - cur_len) > 0)
					my_strlcpy(&vscreen[screen_line][cur_len], EQUATE_STRING, current_columns - (int)cur_len);
			}
 ...
 * or zero on failure.
 */
int flist_equation(int	n)	/* equation space number */
{
	int	sind;
	char	buf[50];
 ...
 */
static int flist_sub(token_type	*p1,			/* expression pointer */
					 int		n,				/* length of expression */
...
static int flist_recurse(token_type	*p1,
						 int		n,
						 int		out_flag,	/* if true, output to gfp or string */
						 char		*string,	/* if not NULL, put output to here instead of gfp */
						 size_t		sbuffer_size,	/* string buffer size */
						 int		line,
						 int		pos,
						 int		cur_level,
						 int		*highp, int *lowp)
