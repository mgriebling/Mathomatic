 ...
 */
int solve_espace(int	want,	/* equation number containing what to solve for */
 ...
 */
int solve_sub(token_type	*wantp,		/* expression to solve for */
 ...
 */
static int poly_solve(long	v)	/* solve variable */
{
	long		i, j, k;
	token_type	*p1, *p2, *ep;
 ...
 */
static int g_of_f(int			op,			/* current operator */
				  ...
				  int			*side2np)	/* pointer to the length of "side2p" */
{
	...
	if (op == POWER && operandp == side1p) {
		if (!parse_complex(side2p, *side2np, &c1))
			return false;
		if (!parse_complex(operandp, (int)operandn, &c2))
			return false;
	...
	if (op == MODULUS) {
		if (get_constant(side2p, *side2np, &d1) && get_constant(operandp, (int)operandn, &d2)) {
			if (fabs(d1) >= fabs(d2)) {
 ...
 * Return true if successful.
 */
static int flip(token_type	*side1p,	/* equation side pointer */
