 ...
 */
void display_startup_message(FILE	*fp)	/* output file pointer */
{
	long	es_size;
 ...
 */
void error(const char	*str)		/* constant string to display */
{
 ...
 */
void warning(const char	*str)		/* constant string to display */
{
 ...
 * There is no return.
 */
void error_bug(const char	*str)	/* constant string to display */
{
 ...
 */
int alloc_espace(int	i)	/* equation space number */
{
 ...
 */
long alloc_to_espace(long	en)	/* equation space number */
{
	...
	for (;;) {
		...
		if (n_equations >= N_EQUATIONS)
			return false;
		if (!alloc_espace((int)n_equations)) {
 ...
 * "dest" is overwritten.
 */
void copy_espace(long src, long dest)	/* equation space numbers */
{
	if (src == dest) {
#if	DEBUG
 ...
 */
int found_var(token_type	*p1,	/* expression pointer */
			  long			n,	/* expression length */
 ...
 */
int var_in_equation(int	 i,	/* equation space number */
					long v)	/* standard Mathomatic variable */
{
	if (empty_equation_space(i))
		return false;
 ...
 */
void rename_var_in_es(int	en,					/* equation space number */
					  long	from_v, long to_v)	/* Mathomatic variables */
{
	int	i;
 ...
 */
int subst_var_with_exp(token_type	*equation,	/* equation side pointer */
 ...
 */
int min_level(token_type	*expression,	/* expression pointer */
			  long			n)				/* expression length */
{
	int		min1;
	token_type	*p1, *ep;
 ...
 * Return -1 on error.
 */
long get_default_en(const char	*cp)
{
	long  i;
 ...
 */
int get_expr(token_type	*equation,	/* where the parsed expression is stored (equation side) */
 ...
 * Return true if successful.
 */
int prompt_var(long	*vp)	/* pointer to the returned variable */
{
 ...
 */
int not_defined(long i)	/* equation space number */
{
	if (i < 0 || i >= n_equations) {
 ...
 */
char *get_string(char	*string,	/* storage for input string */
 ...
 * Return true if successful.
 */
int return_result(long	en)	/* equation space number the result is in */
{
	if (empty_equation_space(en)) {
		return false;
	}
#if	LIBRARY
	make_fractions_and_group((int)en);
	if (factor_int_flag) {
		factor_int_equation((int)en);
	}
	free_result_str();
#if	1	/* Set this to 1 to allow display2d to decide library output mode. */
	if (display2d) {
		result_str = flist_equation_string((int)en);
		if (result_str == NULL)
			result_str = list_equation((int)en, false);
	} else {
		result_str = list_equation((int)en, false);
	}
 ...
 */
int get_range(const char **cpp, long	*ip, long *jp)
{
	long	i;
	char	*cp;
	int	rv;
 ...
 */
int extra_characters(const char	*cp)	/* command line string */
{
	if (cp) {
		cp = skip_comma_space(cp);
		if (*cp) {
 ...
 */
int get_range_eol(const char **cpp, long	*ip, long *jp)
{
 ...
 */
char * skip_space(const char	*cp)	/* character pointer */
{
	if (cp) {
		while (*cp && isspace(*cp))
			cp++;
 ...
 */
char * skip_comma_space(const char	*cp)	/* character pointer */
{
	if (cp) {
		cp = skip_space(cp);
		if (*cp == ',')
 ...
 */
long decstrtol(const char *cp, char **cpp)
{
	long	l;
 ...
 */
int strcmp_tospace(const char *cp1, const char *cp2)
{
	const char	*cp1a, *cp2a;
 ...
 */
int level_plus_count(token_type	*p1,	/* expression pointer */
 ...
 */
int level1_plus_count(token_type	*p1,	/* expression pointer */
					  int			n1)		/* expression length */
{
 ...
 */
int var_count(token_type	*p1,	/* expression pointer */
			  int			n1)		/* expression length */
{
	int	i;
	int	count = 0;
 ...
 */
int no_vars(token_type	*source,	/* expression pointer */
 ...
 */
int exp_contains_infinity(token_type	*p1,	/* expression pointer */
						  int			n1)		/* expression length */
{
	int	i;
 ...
 */
int exp_contains_nan(token_type	*p1,	/* expression pointer */
					 int		n1)	/* expression length */
{
	int	i;
 ...
 */
int exp_is_numeric(token_type	*p1,	/* expression pointer */
				   int			n1)		/* expression length */
{
	int	i;
 ...
 */
int exp_is_absolute(token_type	*p1,	/* expression pointer */
					int			n1)		/* expression length */
{
	int	i;
	int	level;
 ...
 */
char * dirname_win(char	*cp)	/* string containing filename to modify */
{
	int	i;
 ...
 */
int load_rc(int		return_true_if_no_file,
