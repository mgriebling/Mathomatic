...
static int sum_product(const char *cp, enum spf_function current_function);
 ...
 */
int version_cmd(const char *cp)	/* the command-line argument */
{
...
version_report(void)
{
	...
	fprintf(gfp, _("\n%ld equation spaces currently allocated.\n"), n_equations);
...
int solve_cmd(const char *cp)
{
	...
	if ((cp = parse_equation((int)i, cp)) == NULL) {
		return false;
	}
	if (verify_flag) {
	...
	for (k = start; k <= stop; k++) {
		...
		if (verify_flag) {
			pre_v = 0;
			fprintf(gfp, _("Solving equation #%ld for "), cur_equation + 1);
			...
			if (solve_sub(&want, 1, lhs[cur_equation], &n_lhs[cur_equation], rhs[cur_equation], &n_rhs[cur_equation]) > 0) {
				simpa_repeat((int)cur_equation, true, false);	/* Solve result should be quick simplified. */
...
check_result:
				contains_infinity = (exp_contains_infinity(lhs[i], n_lhs[i])
					...
					if (!contains_infinity && once_through < 2) {
						symb_flag = symblify;
						simpa_repeat((int)i, once_through ? false : true, once_through ? true : false);	/* Simplify to compare equation sides. */
						symb_flag = false;
						once_through++;
						goto check_result;
					}
				...
				printf(_("Solve failed for equation space #%ld.\n"), cur_equation + 1);
				rv = 0;
			}
		} else {
			if (solve_espace((int)i, (int)cur_equation)) {
				last_solve_successful = true;
 ...
 */
static int sum_product(const char			*cp,				/* the command-line */
 ...
 */
static int find_more(token_type	*equation,	/* expression to search */
 ...
 * It should be called repeatedly until it returns false.
 */
static int opt_es(token_type	*equation,
				  int			*np)
{
 ...
 */
int optimize_cmd(const char	*cp)
{
	long i, j, k, i1;
	long start, stop;
	...
	do {
		...
		for (i = start; i <= stop; i++) {
			for (j = start; j <= stop; j++) {
				if (i != j) {
					while (find_more(rhs[i], &n_rhs[i], (int)j)) {
						flag = true;
						rv = true;
					}
				}
 ...
 * Return true if successful.
 */
int push_en(int	en)	/* equation space number to push */
{
	char	*cp;
 ...
 * Return true if successful.
 */
int output_current_directory(FILE	*ofp)	/* output file pointer */
{
#if	!SECURE
	char	buf[MAX_CMD_LEN];
 ...
 */
void output_options(FILE	*ofp,			/* output file pointer */
					int		all_set_options)
{
	if (ofp == NULL)
		return;
 ...
 * Return true if successful.
 */
int save_set_options(const char	*cp)
{
	FILE	*fp;
	int	pre_existing;
 ...
 */
int set_options(const char	*cp,
				int		loading_startup_file)
{
	long	i;
	int	negate;
	const char	*cp1 = NULL, *option_string;
...
int copy_cmd(const char	*cp)
{
	...
	do {
		...
		for (i1 = (int)i; i1 <= j; i1++) {
			if (exists[i1]) {
				k = next_espace();
				copy_espace(i1, (int)k);
				if (!return_result(k)) {
					return false;
				}
				if (select_flag) {
 ...
 */
static int complex_func(char	*cp,		/* the command-line */
 ...
 * The real command.
 */
int real_cmd(const char	*cp)
{
	return complex_func((char *)cp, false);
}

/*
 * The imaginary command.
 */
int imaginary_cmd(const char *cp)
{
	return complex_func((char *)cp, true);
}
 ...
 */
static int compare_rhs(int	i, int j,
					   int	*diff_signp)
{
	int	rv;
 ...
 */
int compare_es(int	i, int j)	/* equation space numbers */
{
	int	rv;
...
int compare_cmd(const char *cp)
{
	...
	show_usage = false;
	fprintf(gfp, _("Comparing #%ld with #%ld...\n"), i + 1, j + 1);
	simp_equation((int)i);
	simp_equation((int)j);
	if (n_rhs[i] == 0 || n_rhs[j] == 0) {
		if (n_rhs[i] == 0 && n_rhs[j] == 0) {
			switch (compare_es((int)i, (int)j)) {
			case 1:
...
int divide_cmd(const char	*cp)
{
	...
	if (*cp) {
		input_column += (cp - cp_start);
		cp = parse_expr(rhs[i], &nright, (char *)cp, false);
		if (cp == NULL || nright <= 0) {
			return false;
		}
	}
	if (*cp) {
		...
		cp = skip_comma_space(cp);
		input_column += (cp - cp_start);
		cp = parse_expr(lhs[i], &nleft, (char *)cp, false);
 ...
 */
int eliminate_cmd(const char *cp)
{
...
next_var:
	...
	if (!var_in_equation((int)cur_equation, v)) {
#if	!SILENT
		if (!repeat_flag) {
			list_var(v, 0);
	...
	if (using_flag) {
		...
		for (;; n++) {
			...
			if (used[i])
				continue;
			if (n_lhs[i] && n_rhs[i] && var_in_equation((int)i, v)) {
				if (elim_sub((int)i, v))
					break;
			}
		}
 ...
 */
static int elim_sub(int	i,	/* equation number */
					long v)	/* Mathomatic variable */
{
	token_type	want;
	int		solved;
	...
	simp_equation((int)cur_equation);
	return true;
}
		...
		for (; i <= j; i++) {
			if (n_lhs[i] > 0) {
				...
				if (setjmp(jmp_save) != 0) {	/* trap errors */
					...
					printf("Skipping equation number %ld.\n", i + 1);
					continue;
				}
				fractions_display = new_fractions_display_mode;
				make_fractions_and_group((int)i);
				fractions_display = orig_fractions_display_mode;
				if (factor_flag || factor_int_flag) {
					factor_int_equation((int)i);
				}
				blt(jmp_save, save_save, sizeof(jmp_save));
#if     LIBRARY
				free_result_str();
				result_str = flist_equation_string((int)i);
				if (result_str == NULL)
					result_str = list_equation((int)i, false);
				if (result_str)
					result_en = i;
				if (gfp != stdout) {
					if (flist_equation((int)i) > 0) {
						displayed++;
					}
				}
#else
...
#if	LIBRARY
			free_result_str();
			result_str = list_equation((int)k, export_flag);
			if (result_str)
				result_en = k;
			else
				return false;
...
int code_cmd(const char	*cp)
{
	...
	do {
		...
		for (k = i; k <= j; k++) {
			...
			make_fractions_and_group((int)k);
			if (int_flag) {
...
#if	LIBRARY
			free_result_str();
			result_str = string_code_equation((int)k, language, int_flag);
			if (result_str)
				result_en = k;
			else
				return false;
...
#endif
			if (list_code_equation((int)k, language, int_flag) > 0) {
				displayed = true;
			}
		}
	} while (*cp);
 ...
 */
static int vcmp(const void *v1, const void *v2)
{
	sort_type *p1 = (sort_type *)v1;
	sort_type *p2 = (sort_type *)v2;
 ...
 */
int variables_cmd(const char	*cp)
{
	long		start, stop;
	int			k;
	int			i1;
 ...
 */
int approximate_cmd(const char *cp)
{
	long	start, stop;
	long	k;
	const char	*cp1;
	...
	for (;;) {
		...
		if (vc) {
			...
			if (*cp) {
				...
				if ((cp1 = parse_expr(tes, &n_tes, (char *)cp1, true)) == NULL || n_tes <= 0) {
					return false;
				}
				goto do_this;
			}
 ...
 */
int simplify_cmd(const char	*cp)
{
	int		i, i1;
	long	first, last;
...
int factor_cmd(const char *cp)
{
	...
	if (strcmp_tospace(cp, "number") == 0) {
		...
		valid_range = get_range((const char **)&cp, &first, &last);
		if (!valid_range) {
...
try_parsing:
						input_column += (cp1 - cp_start);
						cp = parse_expr(tes, &n_tes, (char *)cp1, false);
						if (cp == NULL)
							goto retry;
						cp_start = cp;
						if (n_tes <= 0)
 ...
 */
int display_term_count(int	en)	/* equation space number */
{
 ...
 */
int unfactor_cmd(const char	*cp)
{
	long first, last;
	long k;
 ...
 */
int fraction_cmd(const char	*cp)
{
	int	i, div_loc;
	long first, last;
 ...
 */
int read_sub(FILE	*fp,		/* open Mathomatic input file */
