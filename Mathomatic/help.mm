...
typedef	struct {
	...
	int	(*func)(const char *);	/* function that handles this command */
		...
		if (n_lhs[n] == 0 || n_rhs[n] == 0) {
			...
			if (autosolve || equals_flag) {
				if ((n_lhs[n] == 1 && ((lhs[n][0].kind == CONSTANT && lhs[n][0].token.constant == 0.0)
					...
					rv = solve_espace(n, (int)cur_equation);
					n_lhs[n] = 0;
					n_rhs[n] = 0;
					if (rv) {
						return return_result(cur_equation);
 ...
 * Return true if successful.
 */
int display_process(char	*cp)	/* String to process; will be modified, so do not use constant strings. */
{
	long len;
	...
	if (gfp != stdout && gfp != stderr) {
		if (html_flag == 2) {
			set_color(3);
			input_column = fprintf(gfp, "%ld%s", cur_equation + 1, HTML_PROMPT_STR);
			default_color(false);
			input_column -= (strlen(HTML_PROMPT_STR) - strlen(PROMPT_STR));
			fprintf(gfp, "<b>%s</b>", cp);
		} else {
			input_column = fprintf(gfp, "%ld%s", cur_equation + 1, PROMPT_STR);
			fprintf(gfp, "%s", cp);
		}
		if (!nlt)
			fprintf(gfp, "\n");
 ...
 */
int shell_out(char	*cp)	/* shell command string */
{
	int	rv;
 ...
 */
char *parse_var2(long	*vp,	/* pointer to returned variable in Mathomatic internal format */
				 const char	*cp)	/* pointer to variable name string */
{
 ...
 */
int display_usage(const char *pstr,	/* prefix string */
				  int	i)
{
	int	len = 0;
 ...
 */
int display_command(int	i)	/* command table index of command */
{
	int	rows = 2;
...
int read_examples(const char **cpp)
{
	int	i;
	char	*cp;
 ...
 */
void underline_title(int count)	/* length of title, including newline */
{
#if	!NOT80COLUMNS
	int	i;
