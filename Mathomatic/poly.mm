 ...
 */
int poly_in_v_sub(token_type	*p1,			/* expression pointer */
 ...
 */
int poly_in_v(token_type	*p1,			/* expression pointer */
			  int			n,				/* expression length */
			  long			v,				/* Mathomatic variable */
 ...
 */
int poly_factor(token_type	*equation,	/* pointer to the beginning of equation side */
...
static int pf_recurse(token_type	*equation,
 ...
 * Return true if equation side was modified (factored).
 */
static int pf_sub(token_type	*equation,	/* equation side holding the possible polynomial to factor */
				  int			*np,		/* pointer to length of equation side */
...
static int save_factors(token_type	*equation,
						int		*np, int loc1, int len, int level)
{
	int	i, j;
 ...
 */
static int do_gcd(long		*vp)	/* polynomial base variable pointer */
{
	int	i;
	int	count;
 ...
 */
int poly_gcd(token_type	*larger,	/* larger polynomial */
 ...
 */
int poly2_gcd(token_type	*larger,	/* larger polynomial */
			  int			llen,		/* larger polynomial length */
			  token_type	*smaller,	/* smaller polynomial */
			  int			slen,		/* smaller polynomial length */
			  long			v,			/* polynomial base variable */
...
{
	char	*cp;
	int	(*strncmpfunc)(const char*, const char*, size_t);
 ...
 */
int is_integer_expr(token_type	*p1,	/* expression pointer */
					long		n)		/* length of expression */
{
	int	i;
	long	v;
 ...
 */
int mod_simp(token_type	*equation,	/* pointer to the beginning of equation side to simplify */
			 int		*np)		/* pointer to length of the equation side */
{
	return mod_recurse(equation, np, 0, 1);
}

static int mod_recurse(token_type	*equation,
					   int		*np, int loc, int level)
{
	int	modified = false;
	int	i, j, k;
 ...
 */
int poly_gcd_simp(token_type	*equation,
				  int			*np)
{
	return polydiv_recurse(equation, np, 0, 1);
}

static int polydiv_recurse(token_type	*equation,
						   int		*np, int loc, int level)
{
	int	modified = false;
	int	i, j, k;
 ...
 */
int div_remainder(token_type	*equation,
				  int			*np,
...
static int pdiv_recurse(token_type	*equation,
						int	*np, int loc, int level, int code)
{
	int	modified = false;
	int	i, j, k;
	int	op, op2, last_op2;
 ...
 */
int poly_div(token_type	*d1,		/* pointer to dividend */
 ...
 */
static int poly_div_sub(token_type	*d1,
						int			len1,
						token_type	*d2,
						int			len2,
						long		*vp)
 ...
 * Quotient is returned in tlhs[] and remainder in trhs[].
 */
int smart_div(token_type	*d1,		/* pointer to dividend */
			  int			len1,		/* length of dividend */
			  token_type	*d2,		/* pointer to divisor */
			  int			len2)		/* length of divisor */
{
 ...
 */
static int find_highest_count(token_type	*p1,	/* pointer to dividend expression */
...
#if 1
	cv = IMAGINARY + 1; {	// why is loop disabled?
#else
 ...
 */
int find_greatest_power(token_type	*p1,				/* pointer to expression */
						...
						int			*tp1,int *lentp1,	/* the returned term index and length */
