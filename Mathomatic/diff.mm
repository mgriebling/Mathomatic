 ...
 */
int differentiate(token_type	*equation,	/* pointer to source and destination equation side */
 ...
 */
static int d_recurse(token_type	*equation,
 ...
 */
int derivative_cmd(const char *cp)
{
	int		i, len;
	...
	solved = (int)solved_equation((int)cur_equation);
...
/* Simplify and take the first derivative: */
	...
	if (*cp) {
		...
		cp = parse_expr(lhs[our], &our_nlhs, (char *)cp, true);
...
/* make expression into an equation: */
	...
	if (!solved_equation((int)cur_equation)) {
		error(_("The current equation is not solved for a variable."));
		return false;
	}
...
/* parse the command line or prompt: */
	...
	if (*cp) {
		input_column += (cp - cp_start);
		cp = parse_expr(tes, &n_tes, (char *)cp, true);
