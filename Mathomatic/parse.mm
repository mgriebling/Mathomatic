 ...
 */
void put_up_arrow(long    cnt,		/* position of error, relative to "input_column" */
				  const char *cp)	/* error message (constant ASCII string) */
{
#if	!SILENT && !LIBRARY
	int	 i;
 ...
 */
void binary_parenthesize(token_type	*p1,	/* pointer to expression */
						 int		n,	/* length of expression */
						 long		i)	/* location of operator to parenthesize in expression */
{
	long j;
	int	level;
	int	skip_negate;
 ...
 */
void handle_negate(token_type	*equation,
				   int		*np)
{
	int	i;
 ...
 */
void give_priority(token_type	*equation,	/* pointer to expression */
 ...
 */
char * parse_section(token_type	*equation,	/* where the parsed expression is stored (equation side) */
 ...
 */
char * parse_equation(int	n,		/* equation space number */
					  const char *cp)	/* pointer to the beginning of the equation character string */
{
	if ((cp = parse_expr(lhs[n], &n_lhs[n], (char *)cp, true)) != NULL) {
		if ((cp = parse_expr(rhs[n], &n_rhs[n], (char *)cp, true)) != NULL) {
 ...
 * Returns the new string position, or NULL if error.
 */
char * parse_expr(token_type	*equation,	/* where the parsed expression is stored (equation side) */
				  int			*np,		/* pointer to the returned parsed expression length */
				  char			*cp,		/* string to parse */
 ...
 */
char * parse_var(long	*vp, const char	*cp)
{
	long	i, j;
	long	vtmp;
	...
	for (level = 0, cp1 = (char *)cp, i = 0; *cp1;) {
 ...
 */
void set_error_level(char	*cp)	/* input string */
{
	char	*cp1;
	long	len;
 ...
 */
int subst_constants(token_type	*equation,
					int			*np)
{
	int	i;
	int	modified = false;
 ...
 */
int my_strlcpy(char	*dest, const char *src, size_t n)
{
	long	len, len_src;
