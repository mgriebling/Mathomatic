...
long alloc_to_espace(long en);
long alloc_next_espace(void);
int next_espace(void);
void copy_espace(long src, long dest);
int solved_equation(long i);
int found_var(token_type *p1, long n, long v);
...
int set_options(const char *cp, int loading_startup_file);
...
int calculate_cmd(const char *cp);
...
char *parse_var2(long *vp, const char *cp);
...
void put_up_arrow(long cnt, const char *cp);
int isvarchar(int ch);
int paren_increment(int ch);
int is_mathomatic_operator(int ch);
void binary_parenthesize(token_type *p1, int n, long i);
...
char *parse_equation(int n, const char *cp);
...
int is_integer_expr(token_type *p1, long n);
...
int se_compare(token_type *p1, long n1, token_type *p2, long n2, int *diff_signp);
