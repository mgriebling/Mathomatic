 ...
 */
void make_powers(token_type	*equation,	/* pointer to beginning of equation side */
 ...
 */
int int_dispatch(token_type	*equation,	/* pointer to beginning of equation side to integrate */
				 int		*np,		/* pointer to length of equation side */
 ...
 * Return true if successful.
 */
static int integrate_sub(token_type	*equation,	/* pointer to beginning of equation side */
						 int		*np,		/* pointer to length of equation side */
 ...
 */
int integrate_cmd(const char	*cp)
{
	int		i, j;
	int		len;
	long		v = 0;
	...
	n_tlhs = 0;
	n_trhs = 0;
	solved = solved_equation((int)cur_equation);
	i = next_espace();
	...
	if (*cp) {
		...
		if (*cp) {
			integrate_order = strtod(cp, (char **)&cp);
		}
	...
	if (*cp) {
		...
		cp = parse_expr(tlhs, &n_tlhs, (char *)cp, false);
		if (cp == NULL || n_tlhs <= 0) {
			return false;
		}
	}
	if (*cp) {
		...
		cp = skip_comma_space(cp);
		input_column += (cp - cp_start);
		cp = parse_expr(trhs, &n_trhs, (char *)cp, false);
 ...
 * Return true if successful.
 */
static int laplace_sub(token_type	*equation,
					   int			*np,
					   int			loc, int eloc,
					   long			v)
{
 ...
 * Return true if successful.
 */
static int inv_laplace_sub(token_type	*equation,
						   int		*np,
						   int		loc, int eloc,
						   long		v)
{
 ...
 */
int nintegrate_cmd(const char *cp)
{
	long		v = 0;			/* Mathomatic variable */
	int		i, j, k, i1, i2;
	int		len;
	int		level;
	long	iterations = 1000;	/* must be even */
	int		first_size = 0;
	...
	if (*cp) {
		...
		if (*cp) {
			iterations = decstrtol(cp, (char **)&cp);
		}
